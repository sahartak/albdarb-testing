...
class Test_model extends CI_Model
{
    ...
    public function get_test_details($id, $where = null)
    {
        $id = abs((int)$id);
        $test = array();
        $this->db->select('id, name, time, display_mode, created, description, is_public, point, is_open, diff_1_total, diff_2_total, diff_3_total, diff_4_total, diff_5_total,
    ...
    public function get_test_categories($test_id, $appendCounts = false)
    {
        $query = $this->db->get_where('categories', array('test_id' => $test_id));
        $categories = $query->result_array();
        if ($appendCounts) {
            foreach ($categories as &$category) {
                $questionsCounts = $this->db
                    ->select(['difficulty', 'COUNT(*) as total'])
                    ->group_by('difficulty')
                    ->get_where('questions', ['category' => $category['id']])
                    ->result_array();
                $counts = [];
                for ($i = 1; $i<=5; $i++) {
                    $counts[$i] = 0;
                }
                foreach ($questionsCounts as $questionsCount) {
                    $counts[$questionsCount['difficulty']] = $questionsCount['total'];
                }
                $category['questions_counts'] = $counts;
            }
        }
        return $categories;
    }
